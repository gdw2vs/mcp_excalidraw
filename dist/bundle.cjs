...
var LOG_FILE_PATH = process.env.LOG_FILE_PATH || "excalidraw.log";
var logger = import_winston.default.createLogger({
  level: process.env.LOG_LEVEL || "info",
  format: import_winston.default.format.combine(
    import_winston.default.format.timestamp({ format: "YYYY-MM-DD HH:mm:ss.SSS" }),
    import_winston.default.format.uncolorize(),
    import_winston.default.format.metadata({ fillExcept: ["message", "level", "timestamp"] }),
    import_winston.default.format.printf((info) => {
      const extra = info.metadata && Object.keys(info.metadata).length ? ` ${JSON.stringify(info.metadata)}` : "";
...
function normalizeFontFamily(fontFamily) {
  if (fontFamily === void 0) return void 0;
...
var ALLOWED_EXPORT_DIR = process.env.EXCALIDRAW_EXPORT_DIR || process.cwd();
function sanitizeFilePath(filePath) {
  const resolved = import_path.default.resolve(filePath);
  const allowedDir = import_path.default.resolve(ALLOWED_EXPORT_DIR);
  if (!resolved.startsWith(allowedDir + import_path.default.sep) && resolved !== allowedDir) {
    throw new Error(
      `Path traversal blocked: "${filePath}" resolves outside the allowed directory "${allowedDir}". Set EXCALIDRAW_EXPORT_DIR to change the allowed base directory.`
    );
  }
  return resolved;
}
var EXPRESS_SERVER_URL = process.env.EXPRESS_SERVER_URL || "http://localhost:3000";
var ENABLE_CANVAS_SYNC = process.env.ENABLE_CANVAS_SYNC !== "false";
async function syncToCanvas(operation, data) {
  if (!ENABLE_CANVAS_SYNC) {
    logger_default.debug("Canvas sync disabled, skipping");
    return null;
  }
  try {
    let url;
    ...
    switch (operation) {
      ...
      case "batch_create":
        ...
        break;
      default:
        logger_default.warn(`Unknown sync operation: ${operation}`);
        return null;
    }
    logger_default.debug(`Syncing to canvas: ${operation}`, { url, data });
    const response = await fetch(url, options);
    const result = await response.json();
    if (!response.ok) {
      logger_default.warn(`Canvas sync returned error status: ${response.status}`, result);
      throw new Error(result.error || `Canvas sync failed: ${response.status} ${response.statusText}`);
    }
    logger_default.debug(`Canvas sync successful: ${operation}`, result);
    return result;
  } catch (error) {
    logger_default.warn(`Canvas sync failed for ${operation}:`, error.message);
    return null;
  }
}
...
async function getElementFromCanvas(elementId) {
  if (!ENABLE_CANVAS_SYNC) {
    logger_default.debug("Canvas sync disabled, skipping fetch");
    return null;
  }
  try {
    const response = await fetch(`${EXPRESS_SERVER_URL}/api/elements/${elementId}`);
    if (!response.ok) {
      logger_default.warn(`Failed to fetch element ${elementId}: ${response.status}`);
      return null;
    }
    const data = await response.json();
    return data.element || null;
  } catch (error) {
    logger_default.error("Error fetching element from canvas:", error);
    return null;
  }
}
var sceneState = {
...
var PointObjectSchema = external_exports.object({ x: external_exports.number(), y: external_exports.number() });
var PointTupleSchema = external_exports.tuple([external_exports.number(), external_exports.number()]);
var PointSchema = external_exports.union([PointObjectSchema, PointTupleSchema]);
...
var ElementSchema = external_exports.object({
  ...
  type: external_exports.enum(Object.values(EXCALIDRAW_ELEMENT_TYPES)),
  ...
  fontFamily: external_exports.union([external_exports.string(), external_exports.number()]).optional(),
  groupIds: external_exports.array(external_exports.string()).optional(),
  locked: external_exports.boolean().optional(),
  strokeStyle: external_exports.string().optional(),
  roundness: external_exports.object({ type: external_exports.number(), value: external_exports.number().optional() }).nullable().optional(),
  fillStyle: external_exports.string().optional(),
  elbowed: external_exports.boolean().optional(),
  startElementId: external_exports.string().optional(),
  endElementId: external_exports.string().optional(),
  endArrowhead: external_exports.string().optional(),
  startArrowhead: external_exports.string().optional()
});
var ElementIdSchema = external_exports.object({
  id: external_exports.string()
});
var ElementIdsSchema = external_exports.object({
  elementIds: external_exports.array(external_exports.string())
});
var GroupIdSchema = external_exports.object({
  groupId: external_exports.string()
});
var AlignElementsSchema = external_exports.object({
  elementIds: external_exports.array(external_exports.string()),
  alignment: external_exports.enum(["left", "center", "right", "top", "middle", "bottom"])
});
var DistributeElementsSchema = external_exports.object({
  elementIds: external_exports.array(external_exports.string()),
  direction: external_exports.enum(["horizontal", "vertical"])
});
var QuerySchema = external_exports.object({
  type: external_exports.enum(Object.values(EXCALIDRAW_ELEMENT_TYPES)).optional(),
  filter: external_exports.record(external_exports.any()).optional()
});
var ResourceSchema2 = external_exports.object({
  resource: external_exports.enum(["scene", "library", "theme", "elements"])
});
var DIAGRAM_DESIGN_GUIDE = `# Excalidraw Diagram Design Guide
...
- Shapes: 160\xD780 rectangles for services, 120\xD760 for small components
...
### Flowchart
- Shapes: 140\xD770 rectangles for steps, 100\xD7100 diamonds for decisions
...
### ER Diagram
- Shapes: 180\xD740 per entity (wider for attribute lists)
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
  try {
    ...
    logger_default.info(`Handling tool call: ${name}`);
    switch (name) {
      case "create_element": {
        const params = ElementSchema.parse(args);
        logger_default.info("Creating element via MCP", { type: params.type });
        const { startElementId, endElementId, id: customId, ...elementProps } = params;
        const id = customId || generateId();
        const element = {
          id,
          ...elementProps,
          points: elementProps.points ? normalizePoints(elementProps.points) : void 0,
        ...
        logger_default.info("Element created via MCP and synced to canvas", {
...
\u2705 Synced to canvas`
      ...
      case "update_element": {
        ...
        const updatePayload = {
          id,
          ...updates,
          points: rawPoints ? normalizePoints(rawPoints) : void 0,
          updatedAt: (/* @__PURE__ */ new Date()).toISOString()
        };
        if (updatePayload.fontFamily !== void 0) {
        ...
        logger_default.info("Element updated via MCP and synced to canvas", {
          id: excalidrawElement.id,
          synced: !!canvasElement
        });
        return {
...
\u2705 Synced to canvas`
      ...
      case "delete_element": {
        ...
        logger_default.info("Element deleted via MCP and synced to canvas", result);
        return {
          content: [{
            type: "text",
...
\u2705 Synced to canvas`
      ...
      case "get_resource": {
        const params = ResourceSchema2.parse(args);
        const { resource } = params;
        logger_default.info("Getting resource", { resource });
        let result;
        switch (resource) {
          case "scene":
            result = {
      ...
      case "group_elements": {
        ...
        try {
          ...
          const successCount = results.filter((result2) => result2).length;
          if (successCount === 0) {
            sceneState.groups.delete(groupId);
            throw new Error("Failed to group any elements: HTTP server unavailable");
          }
          logger_default.info("Grouping elements", { elementIds, groupId, successCount });
      ...
      case "ungroup_elements": {
        ...
        try {
          ...
          const updatePromises = (elementIds ?? []).map(async (id) => {
            const element = await getElementFromCanvas(id);
            if (!element) {
              logger_default.warn(`Element ${id} not found on canvas, skipping ungroup`);
              return null;
            }
          ...
          const results = await Promise.all(updatePromises);
          const successCount = results.filter((result2) => result2 !== null).length;
          if (successCount === 0) {
            throw new Error("Failed to ungroup: no elements were updated (elements may not exist on canvas)");
          }
          logger_default.info("Ungrouping elements", { groupId, elementIds, successCount });
      ...
      case "align_elements": {
        ...
        logger_default.info("Aligning elements", { elementIds, alignment });
      ...
      case "distribute_elements": {
        ...
        logger_default.info("Distributing elements", { elementIds, direction });
      ...
      case "create_from_mermaid": {
        const params = external_exports.object({
          mermaidDiagram: external_exports.string(),
          config: external_exports.object({
            startOnLoad: external_exports.boolean().optional(),
            flowchart: external_exports.object({
              curve: external_exports.enum(["linear", "basis"]).optional()
            }).optional(),
            themeVariables: external_exports.object({
              fontSize: external_exports.string().optional()
            }).optional(),
            maxEdges: external_exports.number().optional(),
            maxTextSize: external_exports.number().optional()
          }).optional()
        }).parse(args);
        logger_default.info("Creating Excalidraw elements from Mermaid diagram via MCP", {
        ...
        try {
          ...
          const result = await response.json();
          logger_default.info("Mermaid diagram sent to frontend for conversion", {
...
\u26A0\uFE0F  Note: The actual conversion happens in the frontend canvas with DOM access. Open the canvas at ${EXPRESS_SERVER_URL} to see the diagram rendered.`
      ...
      case "batch_create_elements": {
        const params = external_exports.object({ elements: external_exports.array(ElementSchema) }).parse(args);
        logger_default.info("Batch creating elements via MCP", { count: params.elements.length });
        ...
        logger_default.info("Batch elements created via MCP and synced to canvas", {
          count: result.count,
...
${result.syncedToCanvas ? "\u2705 All elements synced to canvas" : "\u26A0\uFE0F  Canvas sync failed (elements still created locally)"}`
      ...
      case "clear_canvas": {
        logger_default.info("Clearing canvas via MCP");
...
${JSON.stringify(data, null, 2)}`
      ...
      case "export_scene": {
        ...
        logger_default.info("Exporting scene via MCP");
        const response = await fetch(`${EXPRESS_SERVER_URL}/api/elements`);
        if (!response.ok) {
        ...
        if (params.filePath) {
          ...
          import_fs.default.writeFileSync(safePath, jsonString, "utf-8");
          return {
            content: [{
              type: "text",
      ...
      case "import_scene": {
        const params = external_exports.object({
          ...
          mode: external_exports.enum(["replace", "merge"])
        }).parse(args);
        logger_default.info("Importing scene via MCP", { mode: params.mode });
        let sceneData;
        if (params.filePath) {
          const safeImportPath = sanitizeFilePath(params.filePath);
          const fileContent = import_fs.default.readFileSync(safeImportPath, "utf-8");
...
\u2705 Synced to canvas`
      ...
      case "export_to_image": {
        const params = external_exports.object({
          format: external_exports.enum(["png", "svg"]),
          filePath: external_exports.string().optional(),
          background: external_exports.boolean().optional()
        }).parse(args);
        logger_default.info("Exporting to image via MCP", { format: params.format });
        ...
        if (params.filePath) {
          ...
          if (params.format === "svg") {
            import_fs.default.writeFileSync(safeImagePath, result.data, "utf-8");
          } else {
            import_fs.default.writeFileSync(safeImagePath, Buffer.from(result.data, "base64"));
          }
          return {
            content: [{
              type: "text",
        ...
        return {
          content: [{
            type: "text",
            text: params.format === "svg" ? result.data : `Base64 ${params.format} data (${result.data.length} chars). Use filePath to save to disk.`
          }]
        };
      }
      case "duplicate_elements": {
        ...
        logger_default.info("Duplicating elements via MCP", { count: params.elementIds.length });
        const duplicates = [];
        for (const id of params.elementIds) {
          const original = await getElementFromCanvas(id);
          if (!original) {
            logger_default.warn(`Element ${id} not found, skipping duplicate`);
            continue;
          }
...
\u2705 Synced to canvas`
      ...
      case "snapshot_scene": {
        const params = external_exports.object({ name: external_exports.string() }).parse(args);
        logger_default.info("Saving snapshot via MCP", { name: params.name });
...
${JSON.stringify(result, null, 2)}`
      ...
      case "restore_snapshot": {
        const params = external_exports.object({ name: external_exports.string() }).parse(args);
        logger_default.info("Restoring snapshot via MCP", { name: params.name });
...
\u2705 Canvas updated`
      ...
      case "describe_scene": {
        logger_default.info("Describing scene via MCP");
        const response = await fetch(`${EXPRESS_SERVER_URL}/api/elements`);
        if (!response.ok) {
        ...
        lines.push(`Types: ${Object.entries(typeCounts).map(([t2, c]) => `${t2}(${c})`).join(", ")}`);
      ...
      case "get_canvas_screenshot": {
        ...
        logger_default.info("Taking canvas screenshot via MCP");
        const response = await fetch(`${EXPRESS_SERVER_URL}/api/export/image`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({
      ...
      case "export_to_excalidraw_url": {
        let makeBaseElement2 = function(el, rest) {
          return {
            ...rest,
            angle: rest.angle ?? 0,
            ...
            roundness: rest.roundness ?? (el.type === "rectangle" || el.type === "diamond" || el.type === "ellipse" ? { type: 3 } : null),
        ...
        var makeBaseElement = makeBaseElement2, concatBuffers = concatBuffers2;
        logger_default.info("Exporting to excalidraw.com URL");
        ...
        for (const el of urlExportElements) {
          ...
          const base = makeBaseElement2(el, rest);
          if (el.type === "text") {
            base.text = text ?? "";
        ...
        const shapeBoundArrows = /* @__PURE__ */ new Map();
        ...
        const innerData = concatBuffers2(fileMetadata, dataBytes);
        const compressed = (0, import_zlib.deflateSync)(Buffer.from(innerData));
        const cryptoKey = await import_crypto.webcrypto.subtle.generateKey(
          { name: "AES-GCM", length: 128 },
          true,
          ["encrypt"]
        );
        const iv = import_crypto.webcrypto.getRandomValues(new Uint8Array(12));
        const encrypted = await import_crypto.webcrypto.subtle.encrypt(
          { name: "AES-GCM", iv },
          cryptoKey,
          compressed
        );
        ...
        const payload = concatBuffers2(encodingMeta, iv, ciphertext);
        ...
        const jwk = await import_crypto.webcrypto.subtle.exportKey("jwk", cryptoKey);
...
Anyone with this link can view and edit the diagram.`
      ...
      case "set_viewport": {
        const viewportParams = external_exports.object({
          scrollToContent: external_exports.boolean().optional(),
          scrollToElementId: external_exports.string().optional(),
          zoom: external_exports.number().min(0.1).max(10).optional(),
          offsetX: external_exports.number().optional(),
          offsetY: external_exports.number().optional()
        }).parse(args || {});
        logger_default.info("Setting viewport via MCP", viewportParams);
...
${JSON.stringify(viewportResult, null, 2)}`
    ...
    logger_default.error(`Error handling tool call: ${error.message}`, { error });
    return {
...
server.setRequestHandler(ListToolsRequestSchema, async () => {
  logger_default.info("Listing available tools");
  return { tools };
});
async function runServer() {
  try {
    logger_default.info("Starting Excalidraw MCP server...");
    const transport = new StdioServerTransport();
    logger_default.debug("Connecting to stdio transport...");
    await server.connect(transport);
    logger_default.info("Excalidraw MCP server running on stdio");
...
process.on("unhandledRejection", (reason, promise) => {
  logger_default.error("Unhandled promise rejection:", reason);
  process.stderr.write(`UNHANDLED REJECTION: ${reason}
`);
  setTimeout(() => process.exit(1), 1e3);
});
if (process.env.DEBUG === "true") {
  logger_default.debug("Debug mode enabled");
}
runServer().catch((error) => {
  logger_default.error("Failed to start server:", error);
  process.exit(1);
});
var index_default = runServer;
